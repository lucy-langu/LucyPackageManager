...
namespace UI {
 ...
 ╚══════╝╚═╝     ╚═╝     ╚═╝
    ...
    void minimalLoader(const std::string& task, int ms_duration) {
        const int width = 35;
        std::cout << CYAN << " ⚡ " << RESET << DIM << task << " " << RESET;
        for (int i = 0; i <= width; ++i) {
            std::cout << "\r" << CYAN << " ⚡ " << RESET << DIM << task << "  " << RESET;
            std::cout << GRAY << "[" << CYAN;
            for (int j = 0; j < i; ++j) std::cout << "━";
            for (int j = i; j < width; ++j) std::cout << " ";
            std::cout << GRAY << "] " << YELLOW << std::setw(3) << (i * 100 / width) << "%" << RESET;
            std::cout.flush();
            std::this_thread::sleep_for(std::chrono::milliseconds(ms_duration / width));
        }
        std::cout << "\n\n";
    }
...
int main(int argc, char* argv[]) {
    ...
    if (uname(&buffer) != 0 || std::string(buffer.sysname) != "Linux") {
        UI::logError("LPM is exclusively engineered for Linux systems.");
        return 1;
    }
